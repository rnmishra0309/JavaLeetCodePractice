...
public class MaxHeap {
	...
	static final Logger logger = LogManager.getLogger(MaxHeap.class);
	...
	private void heapifyDown(int index) {
		...
		int largest = index;
		if(left < size() && heap.get(left) > heap.get(index)) largest = left;
		if(right < size() && heap.get(right) > heap.get(index)) largest = right;
		if(largest != index) {
			swap(largest, index);
			heapifyDown(largest);
		}
